...
const chatWidth = localStorage.getItem('chat-panel-width')
const chatPanelWidth = typeof +chatWidth === 'number' ? Math.max(+chatWidth, 220) : 280
...
const mutations = {
  ...
  SET_CHAT_PANEL_WIDTH (state, width) {
    localStorage.setItem('chat-panel-width', Math.max(+width, 220))
...
const actions = {
  ...
  CHANGE_CHAT_PANEL_WIDTH ({ commit }, width) {
    commit('SET_CHAT_PANEL_WIDTH', width)
  }
}
